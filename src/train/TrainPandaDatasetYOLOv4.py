...
if os.path.exists(imageDirectory) and os.path.exists(trainTXT) and os.path.exists(valTXT) and os.path.exists(
        classesTXT):
...
subprocess.run(['wget', "-O", os.path.join(workingDirectory, "cfg/yolov4-custom1.cfg"),
                "https://raw.githubusercontent.com/roboflow-ai/darknet/master/cfg/yolov4-custom1.cfg"])
subprocess.run(['wget', "-O", os.path.join(workingDirectory, "cfg/yolov4-custom2.cfg"),
                "https://raw.githubusercontent.com/roboflow-ai/darknet/master/cfg/yolov4-custom2.cfg"])
subprocess.run(['wget', "-O", os.path.join(workingDirectory, "cfg/yolov4-custom3.cfg"),
                "https://raw.githubusercontent.com/roboflow-ai/darknet/master/cfg/yolov4-custom3.cfg"])
subprocess.run(['wget', "-O", os.path.join(workingDirectory, "cfg/yolov4-custom4.cfg"),
                "https://raw.githubusercontent.com/roboflow-ai/darknet/master/cfg/yolov4-custom4.cfg"])
subprocess.run(['wget', "-O", os.path.join(workingDirectory, "cfg/yolov4-custom5.cfg"),
                "https://raw.githubusercontent.com/roboflow-ai/darknet/master/cfg/yolov4-custom5.cfg"])
...
# of max_batches, f.e. steps=4800,5400
if os.path.exists(os.path.join(workingDirectory, 'cfg/custom-yolov4-detector.cfg')): os.remove(
    os.path.join(workingDirectory, 'cfg/custom-yolov4-detector.cfg'))
...
with open(os.path.join(workingDirectory, 'cfg/custom-yolov4-detector.cfg'), 'a') as f:
    ...
    # Instructions from the darknet repo change line classes=80 to your number of objects in each of 3 [yolo]-layers:
...
subprocess.run([os.path.join(workingDirectory, "build", 'darknet'), 'detector', 'train',
                os.path.join(workingDirectory, 'data/obj.data'),
                os.path.join(workingDirectory, 'cfg/custom-yolov4-detector.cfg'), weightsFile, '-dont_show'])
